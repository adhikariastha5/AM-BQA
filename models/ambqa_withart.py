...
from models.art import ART,ARTTransformerBlock
from torch import nn
from einops import rearrange

import matplotlib.pyplot as plt

from models.attentionblockfromrestormer import OverlapPatchEmbed,TransformerBlock
import numpy as np
...
class MultiHeadDualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        B, C, N = x.shape
        q = self.c_q(x)
        k1 = self.c_q(x).transpose(-2,-1)
        k2= self.c_v(x).transpose(-2,-1)
        v = self.c_v(x)
        k=k1+k2

        attn = q @ k * self.norm_fact
        attn = self.softmax(attn)
        x = (attn @ v).transpose(1, 2).reshape(B, C, N)
        x = self.proj_drop(x)
        x = x + _x
...
class AMBQA(nn.Module):
    def __init__(self, embed_dim=72, num_outputs=1, patch_size=8, drop=0.1, 
        ...
        bias = False
        #..............
        self.patch_embed = OverlapPatchEmbed(3072, dim)
        self.encoder_level1 = nn.Sequential(*[TransformerBlock(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])
        
        #..................
        #Adding second stage transpose attention
        dim1=384
        #..............
        self.patch_embed2 = OverlapPatchEmbed(768, dim1)
        # self.patch_embed2 = OverlapPatchEmbed(768,)
        self.encoder_level2 = nn.Sequential(*[TransformerBlock(dim=dim1, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])
        ...
        for i in range(num_tab):
            tab = MultiHeadDualBlock(self.input_size ** 2)
            self.tablock1.append(tab)

...
class AMBQA(nn.Module):
    def __init__(self, embed_dim=72, num_outputs=1, patch_size=8, drop=0.1, 
        ...
        self.swin_size=self.input_size // 2
    ...
    def forward(self, x):
        ...
        # stage 1
        x = rearrange(x, 'b (h w) c -> b c (h w)', h=self.swin_size, w=self.swin_size)        
        for tab in self.tablock1:
            x = tab(x)
        x = rearrange(x, 'b c (h w) -> b c h w', h=self.swin_size, w=self.swin_size)

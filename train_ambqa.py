...
# from models.ambqa import AMBQA
# from models.swinonlyambqa import AMBQA
# from models.modify_ambqa import AMBQA
# from models.ambqa_withaddnattention import AMBQA
from models.ambqa_withart import AMBQA
...
def eval_epoch(config, epoch, net, criterion, test_loader):
    with torch.no_grad():
        ...
        for data in tqdm(test_loader):
            ...
            # print('imgname',data['d_img_org'])
            # print('gt label',labels)
            # print('predicted label',pred)
            gd_truth.append(labels)
        ...
        logging.info('Epoch:{} ===== loss:{:.4} ===== SRCC:{:.4} ===== PLCC:{:.4} =====GD:{}====Predict:{}'.format(epoch + 1, np.mean(losses), rho_s, rho_p, gd_truth,pred_labels))
...
if __name__ == '__main__':
    ...
    config = Config({
        ...
        "model_name": "singlekeyconv",
        # "model_name": "try",
        "type_name": "Kadid10k",
        "ckpt_path": "./singlekeyconv/models/",               # directory for saving checkpoint
        "log_path": "./singlekeyconv/log/",
        "log_file": ".log",
        "tensorboard_path": "./singlekeyconv/tensorboard/"
    })
    ...
    # print('dis_train_path:',dis_train_path)
    # print('label_train_path:',label_train_path)
    ...
    # model defination
    net = AMBQA(embed_dim=config.embed_dim, num_outputs=config.num_outputs, dim_mlp=config.dim_mlp,
